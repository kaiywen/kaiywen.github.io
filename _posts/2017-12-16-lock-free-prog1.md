...
简单的分析一下可知，`Update` 函数并不满足 wait-free 的要求。如果多个线程同时执行 `Update` 操作，那么某些线程可能会一直处于循环中；但在整个过程中，部分线程一定能够顺利地完成操作，从而保证程序整体能够继续执行。

对于 go 或者 java 这样天生具有自动垃圾回收机制的语言来说，代码写到这儿可能已经足够了。然而，C++ 需要我们手动的来管理内存，因此你会发现 `pOldMap` 指向的内存块并没有被释放。那么，**什么时候、采用什么机制来释放旧的数据结构成为了横亘在我们面前最大的问题**。
